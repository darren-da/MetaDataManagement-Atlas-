...
var sysCommand={
    ...
    initDetail:function(detailId){
        ...
        system.open(endDiv.html(),{title:"详细信息[IP/system/api/{apicode}?参数..]",area:["600px","650px"]},function(){
	 ...
	 */
	saveDomain:function(){
		system.open("<div id='lp'>"+$('#tkAddCommand').html()+"</div>",{title:"新增命令",area:["600px","550px"]},function(layern,index){
				layui.use(['form','jquery',"laydate"], function() {
					var form = layui.form;
					var laydate = layui.laydate;
					//需要延时
	...
	updateDomain:function(domainID){
		...
		system.open("<div id='lp'>"+$('#tkUpdateCommand').html()+"</div>",
            {title:"修改命令",area:["600px","580px"]},function(layern,index) {
				layui.use(['form', 'jquery', "laydate"], function () {
					var form = layui.form;
					// var laydate = layui.laydate;
					//需要延时
